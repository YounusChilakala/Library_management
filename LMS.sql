...
SELECT Branch.library_branch_BranchName AS Branch_Name,  Book.book_Title Book_Name,
	   Borrower.borrower_BorrowerName AS Borrower_Name, Borrower.borrower_BorrowerAddress AS Borrower_Address,
	   Loans.book_loans_DateOut AS Date_Out, Loans.book_loans_DueDate Due_Date
	   FROM tbl_book_loans AS Loans
		...
		WHERE Loans.book_loans_DueDate = CURDATE() AND Branch.library_branch_BranchName = 'Sharpstown';
...
/* #6- Retrieve the names, addresses, and number of books checked out for all borrowers who have more than five books checked out. */
	...
	SELECT Borrower.borrower_BorrowerName AS Borrower_Name, Borrower.borrower_BorrowerAddress AS Borrower_Address,
		   COUNT(Borrower.borrower_BorrowerName) AS Books_Checked_Out
		   FROM tbl_book_loans AS Loans
		   			INNER JOIN tbl_borrower AS Borrower ON Loans.book_loans_CardNo = Borrower.borrower_CardNo
...
/* #7- For each book authored by "Stephen King", retrieve the title and the number of copies owned by the library branch whose name is "Central".*/
	...
	SELECT Branch.library_branch_BranchName AS Branch_Name, Book.book_Title AS Title, Copies.book_copies_No_Of_Copies AS Number_of_Copies
		   FROM tbl_book_authors AS Authors
			...
			WHERE Branch.library_branch_BranchName = 'Centra' AND Authors.book_authors_AuthorName = 'Stephen King';
